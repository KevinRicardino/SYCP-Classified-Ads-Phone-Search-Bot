...
def carregar_contador():
    try:
        with open("telefones.csv", "r", encoding="utf-8") as arquivo:
...
def salvar_telefone(telefone):
    ...
    string_telefone = (
        f"{numero} - ({telefone[0]}) {telefone[1]} {telefone[2]} - {telefone[3]}\n"
    )
...
if __name__ == "__main__":
    ...
    if resposta_busca:
        ...
        if soup_busca:
            ...
            THREADS = []
            for i in range(5):
                t = threading.Thread(target=descobrir_telefones(LINKS))
                THREADS.append(t)
            ...
            # print(f"\n> Lista com os telefones encontrados: {TELEFONES}")